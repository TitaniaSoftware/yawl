...
public final class ResourceManager extends InterfaceBWebsideController {
	...
	synchronized (_ibEventMutex) {
		...
		if (_workItemCache.contains(wir)) {
		    _log.warn("Duplicate post received for new work item [{}]" + " - no further action required.",
			    wir.getID());
		    return;
		}
		if (wir.isAutoTask()) {
	...
	synchronized (_ibEventMutex) {
		...
		// if it has been 'suspended', find it on a 'started' queue & move it
			...
			_workItemCache.updateResourceStatus(cachedWir, WorkItemRecord.statusResourceSuspended);
		    }
		...
		// if it has moved to started status
			...
			if (p != null) {
			    ...
			    _workItemCache.updateResourceStatus(cachedWir, WorkItemRecord.statusResourceStarted);
			}
			_workItemCache.updateStatus(cachedWir, newStatus);
		    }
		}
     ...
     * event, because it needs the engine to be completely initialised first. It may
     * also be executed via the 'synch' button on the admin queues.
     */
	...
	try {
		...
		if (!_workItemCache.containsKey(wir.getID())) {
		    ...
		    _log.info(
			    "Engine workItem '{}' was missing from local cache and "
				    + "so will be added and distributed according to its " + "resourcing parameters.",
			    wir.getID());
	...
	for (String missingID : missingIDs) {
	    ...
	    _log.info("Cached workitem '{}' did not exist in the Engine and so was removed.", missingID);
	}
    }
	...
	if (wir.isDeferredChoiceGroupMember()) {
	    if (isDeferredChoiceHandled(wir))
		return wir;
	...
	if (rMap != null && rMap.getStartInteraction().getInitiator() == AbstractInteraction.SYSTEM_INITIATED) {
	...
	if (wir.isDeferredChoiceGroupMember()) {
	    _cache.setDeferredGroupHandled(wir.getRootCaseID(), wir.getDeferredChoiceGroupID());
	}
    }

    protected boolean unsetDeferredChoiceHandled(WorkItemRecord wir) {
	return wir.isDeferredChoiceGroupMember()
		&& _cache.unsetDeferredGroupHandled(wir.getRootCaseID(), wir.getDeferredChoiceGroupID());
    }

    protected boolean isDeferredChoiceHandled(WorkItemRecord wir) {
	return wir.isDeferredChoiceGroupMember()
		&& _cache.isDeferredGroupHandled(wir.getRootCaseID(), wir.getDeferredChoiceGroupID());
    }
    ...
    // The strategy is:
    // - Offered: if this is the only p. that has received this offer, give it back
    // to
	...
	if (hasUserTaskPrivilege(p, wir, TaskPrivileges.CAN_DEALLOCATE)) {
	    ...
	    if (wir.isDeferredChoiceGroupMember())
		unsetDeferredChoiceHandled(wir);
    ...
    /**
     * Starts a timer task to refresh the org data dataset at regular intervals, or
     * turns an existing timer off if interval < 0
     *
     * @param interval
	...
	if (completers.isEmpty()) { // empty if tomcat restarted mid-case
		...
		for (XNode eventNode : root.getChildren()) {
		    ...
		    if (!(eTaskID == null || eType == null) && eTaskID.equals(taskID) && eType.equals("complete")) {
	...
	try {
		...
		if (successful(result)) {
		    EventLogger.log(wir, (p != null) ? p.getID() : "", EventLogger.event.complete); // log it
												    // immediately
	...
	if (!_cache.isValidUserSession(handle))
	    return null;
	...
	return _cache.getUserSessionCount();
    }

    public int getAdminSessionCount() {
	return _cache.getAdminSessionCount();
    }

    public int getUserAdminSessionCount() {
	return _cache.getUserAdminSessionCount();
    }
	...
	synchronized (_ibEventMutex) {
		...
		_log.error("Error attempting to cancel case {}: {}", caseID, StringUtil.unwrap(result));
	    }
	...
	_logger.debug(String.format("Trying to get value for %s from case data: %s", paramName, caseData));
	...
	ResourceMap rMap = getResourceMap(wir);
	return (rMap != null && rMap.getAllocateInteraction().getInitiator() == AbstractInteraction.SYSTEM_INITIATED);
    }
	...
	try {
		...
		_log.error("Could not run codelet '{}' for workitem '{}' - error "
			+ "getting task information from engine. Codelet ignored.", codelet, wir.getID());
		checkInAutoTask(wir, wir.getDataList()); // check in immediately
		return;
	    }
	...
	try {
		...
		_log.info("Automated task '{}' successfully processed and checked back " + "into the engine.",
			wir.getID());
	    } else {
		_log.error("Automated task '{}' could not be successfully completed. " + "Result message: {}",
			wir.getID(), msg);
		_log.debug(String.format("dataList:\n %s\n  outData: %s", JDOMUtil.elementToString(wir.getDataList()),
			JDOMUtil.elementToString(outData)));
	    }
	} catch (Exception e) {
     ...
     * @pre The item id refers to a work item that is currently in the list of items
     *      known to the Resource Service, and the work item has enabled or fired
     *      status
