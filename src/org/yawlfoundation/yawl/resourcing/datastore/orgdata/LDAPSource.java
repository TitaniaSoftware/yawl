...
public class LDAPSource extends DataSource {
    ...
    /**
     * Gets the set of attributes we are interested in retrieving for each user from
     * the LDAP server. The map uses a generic key for each attribute and maps it to
     * the actual corresponding attribute name as read from the properties file.
     * 
	...
	if (_attributeMap == null) {
		...
		String mail = getProperty("mail");
		if (isNotNullOrEmpty(mail)) {
		    _attributeMap.put("mail", mail);
		}
	    }
	}
     ...
     * binding read from the properties file. The list is retrieved in a single call
     * to the server.
     * 
     ...
     * @throws NamingException
     *             if something goes wrong when reading role info from the server
     */
	...
	String mail = getStringValue(attributes, "mail");
    ...
    /**
     * Loads the LDAP server-stored password for a Participant, if specified to do
     * so via the properties file.
     * 
     * @param attributes
