...
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_COMMON_NAME;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_DESCRIPTION;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_DISPLAYNAME;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_GIVENNAME;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_MAIL;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_NOTES;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_ORGGROUP_TYPE;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_POSITION_ORG_GROUP;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_POSITION_REPORTS_TO;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_PRIVILEGE_ADMINISTRATOR;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_PRIVILEGE_CAN_CHAIN_EXECUTION;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_PRIVILEGE_CAN_CHOOSE_ITEM_TO_START;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_PRIVILEGE_CAN_MANAGE_CASES;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_PRIVILEGE_CAN_REORDER;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_PRIVILEGE_CAN_START_CONCURRENT;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_PRIVILEGE_CAN_VIEW_ORG_GROUP_ITEMS;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_PRIVILEGE_CAN_VIEW_TEAM_ITEMS;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_SURNAME;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_UID;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_UNIQUE_MEMBER;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ATTR_YAWL_INTERNAL_ID;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.OC_CAPABILITY_UNIQUE_NAMES;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.OC_ORG_GROUP;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.OC_PARTICIPANT;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.OC_POSITION_UNIQUE_NAMES;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.OC_ROLE_UNIQUE_NAMES;
import static org.yawlfoundation.yawl.resourcing.datastore.orgdata.LDAPConstants.ORG_GROUP_TYPES;
...
/**
 * openLDAP DataSource connecting to an openLDAP server using the YAWL LDAP
 * schema.
 * 
...
public class LDAPSourceExtended extends DataSource {
	...
	for (String binding : getProperty("binding").split(";")) {
		...
		try {
		    NamingEnumeration<SearchResult> searchResult = dctx.search(binding, LDAPConstants.OC_FILTER,
			    LDAPConstants.SUBTREE_SCOPE);

		    if (searchResult.hasMore()) {
			while (searchResult.hasMore()) {
			    NameClassPair nc = searchResult.next();
			    _inputMap.put(new LdapName(nc.getNameInNamespace()),
				    dctx.getAttributes(nc.getNameInNamespace()));
			}
		    }
     ...
     * instances like Participant, OrgGroup etc. No relations (belongsTo, reportsTo)
     * are set here.
     * 
	...
	for (Map.Entry<String, Position> positionNode : _positionsWithDNasKey.entrySet()) {
		...
		if (_orgGroupsWithDNasKey.containsKey(orgGroupDN)) {
		    ...
		    _log.error("The orggroup '" + orgGroupDN + "' for position " + positionNode.getKey()
			    + " could not be found.");
		}
	    }
		...
		if (_positionsWithDNasKey.containsKey(reportsToDN)) {
		    ...
		    _log.error("The position '" + reportsToDN + "' for position " + positionNode.getKey()
			    + " could not be found or is not a position.");
		}
	    }
	}
	...
	String mail = getAttributeAsString(attr, ATTR_MAIL);
	...
	UserPrivileges userPrivileges = new UserPrivileges(participant.getID(), canChooseItemToStart,
		canStartConcurrent, canReorder, false, false, false, canViewTeamItems, canViewOrgGroupItems,
	...
	if (ORG_GROUP_TYPES.contains(typeUnvalidated)) {
	    ...
	    _log.error("Group type '" + typeUnvalidated + "' of group " + cn
		    + " is not valid. Seting value to default 'GROUP'.");
	}
	...
	try {
	    ...
	    is = Thread.currentThread().getContextClassLoader().getResourceAsStream("LDAPSourceExtended.properties");
	    if (is == null)
		throw new Exception("LDAPSourceExtended.properties not found.");
	...
	if (_environment == null) {
		...
		String url = String.format("%s://%s:%s", scheme, getProperty("host"), getProperty("port"));
	...
	if (_props != null) {
		...
		if (!_capabilitiesWithIDasKey.isEmpty())
		    rds.setCapabilities(_capabilitiesWithIDasKey, this);
		if (!_orgGroupsWithIDasKey.isEmpty())
		    rds.setOrgGroups(_orgGroupsWithIDasKey, this);
		if (!_rolesWithIDasKey.isEmpty())
		    rds.setRoles(_rolesWithIDasKey, this);
		if (!_positionsWithIDasKey.isEmpty())
		    rds.setPositions(_positionsWithIDasKey, this);
    ...
    /*
     * Output for debugging purposes private void printMaps() { for
     * (Map.Entry<String, Participant> participantEntry :
     * _participantsWithDNasKey.entrySet()) {
     * printParticipant(participantEntry.getValue()); } for (Map.Entry<String,
     * OrgGroup> orgGroupEntry : _orgGroupsWithDNasKey.entrySet()) {
     * printOrgGroup(orgGroupEntry.getValue()); } for (Map.Entry<String, Role>
     * roleEntry : _rolesWithDNasKey.entrySet()) { printRole(roleEntry.getValue());
     * } for (Map.Entry<String, Capability> capabilityEntry :
     * _capabilitiesWithDNasKey.entrySet()) {
     * printCapability(capabilityEntry.getValue()); } for (Map.Entry<String,
     * Position> positionEntry : _positionsWithDNasKey.entrySet()) {
     * printPosition(positionEntry.getValue()); } }
     * 
     * private void printParticipant(Participant participant) {
     * System.out.println(""); System.out.println("         Participant");
     ...
     * participant.getUserPrivileges().getPrivilegesAsBits()); }
     * 
     * private void printOrgGroup(OrgGroup orgGroup) { System.out.println("");
     * System.out.println("           OrgGroup"); printAbstractResource(orgGroup);
     * System.out.println("         Group Name " + orgGroup.getGroupName());
     * System.out.println("               Type " + orgGroup.getGroupType());
     * System.out.println("          BelongsTo " + (orgGroup.getBelongsTo() != null
     * ? orgGroup.getBelongsTo().getName() : "")); }
     * 
     * private void printRole(Role role) { System.out.println("");
     * System.out.println("               Role"); printAbstractResource(role);
     * System.out.println("          BelongsTo " + (role.getOwnerRole() != null ?
     * role.getOwnerRole().getName() : ""));
     * System.out.println("            Members " + (role.getResources()!= null ?
     * role.getResources() : "")); }
     * 
     * private void printCapability(Capability capability) { System.out.println("");
     * System.out.println("          Capability");
     * printAbstractResource(capability); System.out.println("            Members "
     * + (capability.getResources()!= null ? capability.getResources() : "")); }
     * 
     * private void printPosition(Position position) { System.out.println("");
     * System.out.println("            Position"); printAbstractResource(position);
     * System.out.println("          BelongsTo " + (position.getOrgGroup() != null ?
     * position.getOrgGroup().getName() : ""));
     * System.out.println("          ReportsTo " + (position.getReportsTo() != null
     * ? position.getReportsTo().getName() : ""));
     * System.out.println("            Members " + (position.getResources()!= null ?
