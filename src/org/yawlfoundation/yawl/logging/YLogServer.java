...
public class YLogServer {
	...
	String result;
	if (isEnabled()) {
	    Query query = _logDb.createQuery("select ni from YLogNet as n, YLogNetInstance as ni "
		    + "where ni.netID=n.netID and n.specKey=:specKey").setLong("specKey", specKey);
	    if (query != null) {
	...
	String result;
	if (isEnabled()) {
	    Query query = _logDb.createQuery("from YLogEvent as e where " + "e.rootNetInstanceID=:key").setLong("key",
		    rootNetInstanceKey);
	    if (query != null) {
		Iterator itr = query.iterate();
		if (itr.hasNext()) {
	...
	String result;
	if (isEnabled()) {
	    Query query = _logDb.createQuery("from YLogNetInstance as n where n.engineInstanceID=:caseID")
	...
	if (isEnabled()) {
	    Query query = _logDb
		    .createQuery("from YLogTaskInstance as ti, YLogTask as t " + "where (ti.engineInstanceID=:caseID "
			    + "or ti.engineInstanceID like :caseIDlike) " + "and ti.taskID=t.taskID "
			    + "and t.name=:taskName")
		    .setString("caseID", caseID).setString("caseIDlike", caseID + ".%").setString("taskName", taskName);
	    if (query != null) {
		Iterator itr = query.iterate();
		if (itr.hasNext()) {
		    StringBuilder xml = new StringBuilder();
	...
	String result;
	if (isEnabled()) {
	    Query query = _logDb.createQuery("from YLogEvent as e where " + "e.instanceID=:key").setLong("key",
		    instanceKey);
	    if (query != null) {
		Iterator itr = query.iterate();
		if (itr.hasNext()) {
	...
	String result;
	if (isEnabled()) {
	    Query query = _logDb.createQuery("from YLogDataItemInstance as di where " + "di.eventID=:key")
	...
	String result;
	if (isEnabled()) {
	    Query query = _logDb.createQuery("from YLogDataType as dt where " + "dt.dataTypeID=:key").setLong("key",
		    dataTypeKey);
	    if (query != null) {
		Iterator itr = query.iterate();
		if (itr.hasNext()) {
	...
	String result;
	if (isEnabled()) {
	    Query query = _logDb.createQuery("from YLogTaskInstance as ti where " + "ti.engineInstanceID like :key")
	...
	String result;
	if (isEnabled()) {
	    Query query = _logDb.createQuery("from YLogTaskInstance as ti where " + "ti.taskID=:key").setLong("key",
		    taskKey);
	    if (query != null) {
		Iterator itr = query.iterate();
		if (itr.hasNext()) {
	...
	String result;
	if (isEnabled()) {
	    Query query = _logDb.createQuery("select e from YLogEvent as e, YLogNetInstance as ni where "
		    + "ni.engineInstanceID=:caseID and e.instanceID=ni.netInstanceID " + "and e.descriptor=:eventType")
		    .setString("caseID", caseID).setString("eventType", eventType);
	    if (query != null) {
		Iterator itr = query.iterate();
		if (itr.hasNext()) {
		    StringBuilder xml = new StringBuilder();
	...
	if (isEnabled()) {
	    Query query = _logDb
		    ...
		    .setString("serviceName", serviceName).setString("event", eventType);
	    if (query != null) {
		Iterator itr = query.iterate();
		if (itr.hasNext()) {
		    StringBuilder xml = new StringBuilder();
	...
	if (isEnabled()) {
		...
		List instances = getNetInstanceObjects(spec.getRootNetID());
		StringBuilder xml = new StringBuilder();
		// Some internal datatype definitions have been logged without xs prefix
		// declaration. Add it here to catch all cases. [pht 2021-01-14]
		xml.append(
			String.format("<cases specKey=\"%d\" xmlns:xs=\"http://www.w3.org/2001/XMLSchema\">", specKey));
		for (Object o : instances) {
		    YLogNetInstance instance = (YLogNetInstance) o;
	...
	if (isEnabled()) {
	    ...
	    if (identifier != null) {
		query = _logDb
			.createQuery("select distinct s from YLogSpecification s where "
		...
		query = _logDb
			...
			.setString("version", specID.getVersionAsString()).setString("uri", specID.getUri());
	    }
	    if (query != null) {
		Iterator itr = query.iterate();
		if (itr.hasNext()) {
	...
	if (isEnabled()) {
	    Query query = _logDb.createQuery("from YLogNet as n where n.specKey=:specKey").setLong("specKey", specKey);
	    if (query != null) {
		return query.list();
	    }
	}
	...
	if (isEnabled()) {
	    return (YLogNetInstance) _logDb.selectScalar("YLogNetInstance", "parentTaskInstanceID",
		    String.valueOf(key));
	} else
	    return null;
    }
	...
	if (isEnabled()) {
	    Query query = _logDb
		    ...
		    .setString("caseid", caseID).setString("likeid", caseID + ".%");
	    if (query != null) {
		return query.list();
	    }
	}
	...
	if ((itemID != null) && (isEnabled())) {
		...
		Query query = _logDb
			...
			.setString("caseID", caseID).setString("taskName", taskName);
		if (query != null) {
	...
	return getObjects("from YLogEvent as e where e.instanceID=:key " + "order by e.timestamp", key);
    }

    private List getTaskInstanceObjects(long key) {
	return getObjects("from YLogTaskInstance as ti where ti.parentNetInstanceID=:key", key);
    }
