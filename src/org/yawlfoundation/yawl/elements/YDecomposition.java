...
/**
 * 
 * @author Lachlan Aldred Date: 25/09/2003 Time: 16:04:21
 *
...
public abstract class YDecomposition implements Cloneable, YVerifiable {
    ...
    protected static final Logger logger = LogManager.getLogger(YDecomposition.class);
	...
	if (pmgr != null)
	    pmgr.storeObjectFromExternal(_casedata);
    }
    ...
    // if set to true, any task that decomposes to this will need to have a
    // resourcing
    // strategy specified
	...
	// just do the decomposition facts (not the surrounding element) - to keep life
	// simple
	...
	for (String expression : _outputExpressions) {
	    xml.append("<outputExpression query=\"").append(JDOMUtil.encodeEscapes(expression)).append("\"/>");
	}
     ...
     * declared output parameters. Only useful for Beta 4 and above. The data inside
     * this decomposition is groomed so to speak so that the output data is returned
     * in sequence. Furthermore no internal variables, \ or input only parameters
     * are returned.
     * 
	...
	for (YParameter parameter : outputParamsList) {
	    logger.debug(String.format("getting output parameter \"%s\" (preferredName \"%s\")", parameter.getName(),
		    parameter.getPreferredName()));
	    Element child = root.getChild(parameter.getPreferredName());
	    if (null == child) {
		logger.error(String.format("Couldn't find child element named \"%s\"",parameter.getPreferredName()));
	    }
	...
	if (pmgr != null)
	    pmgr.updateObjectExternal(_casedata);
    }
	...
	for (YParameter inputParam : _inputParameters.values()) {
		...
		initialValueXML = new Element(inputParam.getName());
