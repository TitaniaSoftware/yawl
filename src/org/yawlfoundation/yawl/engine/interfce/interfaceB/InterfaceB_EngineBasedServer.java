...
import org.yawlfoundation.yawl.engine.interfce.PerfReporter;
...
public class InterfaceB_EngineBasedServer extends YHttpServlet {
	...
	try {
		...
		_engine = new EngineGatewayImpl(engineImpl, persist, enableHbnStats, redundantMode);
	    ...
	    // set the country/region codes used for calculating work-day-only timers (if
	    // any)
	...
	if (!StringUtil.isNullOrEmpty(implClassName)) {
		...
		_log.warn("Unable to locate external YEngine class '" + implClassName + "'.", e);
	    }
	...
	outputWriter.flush();
	outputWriter.close();
	// todo find out how to provide a meaningful 500 message in the format of a
	// fault message.
    }
	...
	try {
		...
		if (action.equals("checkConnection")) {
			...
			msg.append(_engine.launchCase(specID, caseParams, completionObserver, caseID, logDataStr,
				sessionHandle));
		    } else
			msg.append(
	...
	if (_log.isDebugEnabled()) {
	    _log.debug("\nInterfaceB_EngineBasedServer::do{}() request.getRequestURL={}", service,
